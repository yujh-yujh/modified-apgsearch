# *************************************************
# * Ash Pattern Generator (apgsearch)             *					# not Ash Pattern Groucher!
# *************************************************
# * HACKED for windows users unable to use WSL    *
# *************************************************
# * Version: v0.0.0                               *
# *************************************************
# IMPORTANT: THIS VERSION USES PYTHON 2.7 AND GOLLY 3.3-. SLOWER THAN APGMERA
# NOTICE: Below information may or may not be accurate.
#         Does not support golly 3.4+.
# -- Processes roughly 100 soups per (second . core . GHz) in Life (B3/S23).
#
# -- Can perfectly identify oscillators with period < 1000, not well-separated
...
#
# -- At most 99.9999999% reliable at identifying objects.
#
# -- Scores soups based on the total excitement of the ash objects. -- useless
#
...
#UPDATE CHECKLIST:
#Change date in the filename
#Change version number in the filename
#Change vnum global variable
...
def get_server_address():
    ...
    # Chinese users can now use catagolue.hatsya.com! catagolue.appspot.com is unavalible without a vpn in China.
...
# web browser and visiting http://catagolue.hatsya.com/payosha256
...
def hashsoup(instring, sym):
    ...
    if sym in ['D2_x', 'D8_1', 'D8_4', 'D8_9', 'AB_D2_x_skewgutter_Test']:
        d = 1
    ...
    for j in xrange(jrange):
        ...
        for k in xrange(8):
            ...
            if (t & (1 << (7 - k))):
                ...
                if ((d == 0) | ((x == y) & (sym != 'AB_D2_x_skewgutter_Test')) | (x > y)):
    ...
    if (d >= 1):
        for x in xrange(0, len(thesoup), 2):
            if (sym == 'AB_D2_x_skewgutter_Test'):
...
def multito2(clist):
    ...
    l = l-l%2
    clist = clist[:l]
    return [clist[i] for i in xrange(len(clist)) if i%3 != 2]
...
def decompose(pattern, period, dispx, dispy, alpharule):
    ...
    origrule = g.getrule()
    rect = g.getrect()
    w = rect[2] + 100#!Should test over all gens
    pat = g.getcells(rect)
    ...
    g.setrule("APG_TreeMaker_" + alpharule)
    for i in pats:
    ...
    pats = None
    #offs = None
    while pats != oldpats: #!Need to carry over significant OFF cells through iterations of this loop, otherwise it's often an infinite one (and it doesn't work).
        oldpats = pats
        pats       = decompose_0(g.getcells(g.getrect()), period, dispx, dispy, w, alpharule, False)#, offs)
        #   , offs
        g.setrule("APG_Decayer")
        for i in xrange(len(pats)):
            pats[i] = g.evolve(pats[i], 1)
...
class Soup:
    ...
    commonnames = {"xp2_7": ("blinker", 0),
                   ...
                   "xq6_15": ("[[JustFriends]] (1,1)c/6", 0),
                   "xq6_30d": ("[[JustFriends]] double (1,1)c/6", 0),
                   "xq6_1145gs": ("[[JustFriends]] triple (1,1)c/6", 5),
                   "xq4_15": ("B2ce/S1 (1,1)c/4", 0),
                   "xp2_5": ("Fake blinker", 0)}
    ...
    def census(self, stepsize):
        ...
        self.incobject(self.rg.blcode if self.rg.bl else "xp2_7", (pop2-pop3)/5) #Blinkers have two possible apgcodes, but just in case something wonky's happened, default to the Life one.
...
#ALGO REFS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def bijoscar(self, maxsteps, gracepd, disp=False):
        '''g.setrule(self.rg.slashed)
        ...
        for i in xrange(maxsteps):
            ...
            if (int(g.getpop()) == initpop):
                ...
                if (phash == inithash):
                    ...
                    if (prect == initrect):
                        return period if not disp else (period, 0, 0)
                    else:
                        return -period if not disp else (period, prect[0]-initrect[0], prect[1]-initrect[1])
                        
        return (maxsteps + 1) if not disp else (maxsteps + 1, 0, 0)
    ...
    def enter_unid(self, unidname, soupid, moving):
        ...
        if not(unidname in self.decompositions):
            ...
            if (moving) and self.pseudo:
                ...
                listoflists = decompose(g.getcells(g.getrect()), *self.bijoscar(1000, 4, True), alpharule=self.rg.alphanumeric)

            if not len(listoflists):#True:
            ...
            for livecells in listoflists:
                ...
                if self.rg.ruletype or g2_8:
                    ...
                    g.setalgo("RuleLoader")
                g.setrule(self.rg.slashed)
                #g.getstring(str(livecells))
                g.putcells(livecells)
                period = self.bijoscar(1000, 4)
                canonised = canonise(abs(period))
                if canonised == "#":
                    self.incobject("ov_q"+str(-period), 1)
                    self.awardpoints2(soupid, "ov_q"+str(-period))
                elif (period < 0):
...
#DEFINE RULE-BASED TEST FOR XWSS EXISTENCE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                if ((unidname[0] == 'U') & (unidname[1] == 'S') & (unidname[2] == 'S')):
                    ...
                    if ((unidname[0] == 'x') & (unidname[1] == 'q') & (unidname[3] == '_')) or unidname.startswith("ov_q"):
...
def apg_main():
    ...
    orignumber = int(g.getstring("How many soups to search between successive uploads?", "100000")) if upload else int(g.getstring("How many soups to search?", "5000000"))
    if not upload:
    ...
    if upload:
        # Sanitise input:
        orignumber = max(orignumber, 5000)
	# Catagolue now supports 5000 sopus per haul! but do not do that too often!
        orignumber = min(orignumber, 100000000)
    number = orignumber
    initpos = 0
    if symmstring not in ["AB_256x256_Test", "AB_sha512_16x32_Test", "AB_sha512_20x20_Test", "AB_C1_2x2_32x32_Test", "AB_D2_x_skewgutter_Test", "1x256", "2x128", "4x64", "8x32", "C1", "C2_1", "C2_2", "C2_4", "C4_1", "C4_4", "D2_+1", "D2_+2", "D2_x", "D4_+1", "D4_+2", "D4_+4", "D4_x1", "D4_x4", "D8_1", "D8_4"]:
        g.exit(symmstring+" is not a valid symmetry option")
        
    soup = Soup()
    #Non-totalistic
    if len(filter(lambda c: c in "acdefghijklmnopqrtuvwxyz", rulestring)):
...
def symmetry_test():
    ...
    symmetries = [["32x32", "C1", "8x32", "4x64", "2x128", "1x256"],
                  ["AB_256x256_Test", "AB_sha512_16x32_Test", 'AB_sha512_20x20_Test', 'AB_C1_2x2_32x32_Test', 'AB_D2_x_skewgutter_Test'],
                  ["C2_1", "C2_2", "C2_4"],
                  ["C4_1", "C4_4"],
                  ["D2_+1", "D2_+2", "D2_x"],
