...
#
# -- Not so reliable at identifying objects.
#
...
# == Stolen and modified again by yujh ==
# apgsearch-isotropic - yujh v1.0
# 
...
def hashsoup(instring, sym):
    ...
    if sym in ['D2_x', 'D8_1', 'D8_4', 'D8_9', 'D8_16', 'D8_25', 'D8_49', 'D8_81', 'D8_121', 'D8_169']:
        d = 1
    elif sym in ['D4_x1', 'D4_x4', 'D4_x9']:
        d = 2
    else:
        d = 0
    ...
    if sym in ['C2_1', 'C4_1', 'D8_1', '1x256x2+1']:
        ...
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x])
            thesoup.append(-thesoup[x+1]-1)
    elif sym in ['C2_4', 'C4_4', 'D8_4', '1x256+2']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-1)
            thesoup.append(-thesoup[x+1]-1)
    elif sym in ['D8_9', 'C4_9', '1x256+3']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-2)
            thesoup.append(-thesoup[x+1]-2)
    elif sym in ['D8_16']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-3)
            thesoup.append(-thesoup[x+1]-3)
    elif sym in ['D8_25',]:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-4)
            thesoup.append(-thesoup[x+1]-4)
    elif sym in ['D8_49',]:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-6)
            thesoup.append(-thesoup[x+1]-6)
    elif sym in ['D8_81']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-8)
            thesoup.append(-thesoup[x+1]-8)
    elif sym in ['D8_121']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-10)
            thesoup.append(-thesoup[x+1]-10)
    elif sym in ['D8_169']:
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(-thesoup[x]-12)
            thesoup.append(-thesoup[x+1]-12)

    # Checks for rotate4 symmetry:
    if (sym in ['C4_1', 'D8_1', '1x256_rotate4']):
        for x in xrange(0, len(thesoup), 2):
            thesoup.append(thesoup[x+1])
            thesoup.append(-thesoup[x])
...
soupinarow = int(g.getstring("How many soups in a row?", "10"))
# initpos = int(g.getstring("Initial position: ", "0"))
initpos = 0

if symmstring not in ["8x32", "4x64", "2x128", "1x256", "32x32", "C1", "C2_1", "C2_2", "C2_4", "C4_1", "C4_4", "C4_9", "D2_+1", "D2_+2", "D2_x", "D4_+1", "D4_+2", "D4_+4", "D4_x1", "D4_x4", "D4_x9", "D8_1", "D8_4", "D8_9", "D8_16", "D8_25", "D8_49", "D8_81", "D8_121", "D8_169", "Test1", "1x256_rotate4", "1x256x2+1", "Test2"]:
